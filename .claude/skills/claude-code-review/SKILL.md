---
description: "Comprehensive code review using Claude Opus subagents as the review engine. Use when the user asks for a code review with Claude."
argument-hint: "optional scope: backend, frontend, security, or leave empty for all"
---

# Claude Code Review

Comprehensive code review using Claude Opus subagents as the review engine.
...
Create a TaskCreate entry for **each review area** below (or the filtered subset if `$ARGUMENTS` was provided). Each task should use the review area name as the subject and include the area's scope and focus in the description. Set the `activeForm` to "Reviewing [area name] with Claude".

### Phase 3: Execute Parallel Claude Reviews
...
|---|---|
| Backend Core & State | `claude-backend-reviewer` |
| Task System | `claude-backend-reviewer` |
| Command Handlers | `claude-backend-reviewer` |
| Security & Executors | `claude-security-reviewer` |
| Fleet & SSH | `claude-backend-reviewer` |
| Blueprint System | `claude-backend-reviewer` |
| AppArmor System | `claude-backend-reviewer` |
| Frontend Stores & IPC | `claude-frontend-reviewer` |
| Frontend UI Components | `claude-frontend-reviewer` |
| Cross-Cutting & Tests | `claude-integration-reviewer` |
...
1. Collect findings from all subagent responses.
2. Consolidate into a single `claude-code-review.md` in the project root.
3. Group findings by severity (HIGH first, then MEDIUM, then LOW).
4. Deduplicate any findings that appear in multiple review areas.
5. Add a "Validation" section with current test counts (`cargo test`, `npm run test`) and build status (`cargo check`, `npx tsc --noEmit`).
...
- **Slug**: `backend-core`
- **Subagent**: `claude-backend-reviewer`
...
- **Subagent**: `claude-security-reviewer`
...
- **Subagent**: `claude-frontend-reviewer`
...
- **Slug**: `cross-cutting`
- **Subagent**: `claude-integration-reviewer`
