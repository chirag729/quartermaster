...
import type { SshHostEntry, NodeStatus } from "../types/node";
...
export function FleetPage() {
  ...
  const loadNodes = useCallback(async () => {
    ...
    try {
      ...
      addToast({ type: "error", title: "Failed to load nodes", message: formatError(err) });
    } finally {
      setLoading(false);
    }
  ...
  const pollStatuses = useCallback(async () => {
    setPolling(true);
    try {
      const results = await api.pollAllNodeStatuses();
      for (const [nodeId, isOnline] of results) {
        const status: NodeStatus = isOnline ? "online" : "offline";
        updateNodeStatus(nodeId, status);
      }
    } catch {
      // Silently fail polling -- not worth a toast every 30s
    } finally {
      setPolling(false);
    }
  }, [updateNodeStatus]);
  ...
  // Poll statuses on mount and every 30s with cleanup
  useEffect(() => {
    pollStatuses();
  ...
  const handleDiscoverHosts = async () => {
    ...
    try {
      const hosts = await api.discoverSshHosts();
    ...
    <div className={selectionMode && selectedNodeIds.length > 0 ? "pb-20" : ""}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => { loadNodes(); pollStatuses(); }} disabled={loading}>
            <RefreshCw size={14} className={loading || polling ? "animate-spin" : ""} />
            Refresh
          </Button>
