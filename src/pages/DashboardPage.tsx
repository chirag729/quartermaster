...
export function DashboardPage() {
  ...
  const { nodes, loading: nodesLoading, setNodes, setLoading: setNodesLoading } = useFleetStore();
  const { blueprints, loading: blueprintsLoading, setBlueprints, setLoading: setBlueprintsLoading } = useBlueprintStore();
  const { addToast } = useToastStore();

  const loading = nodesLoading || blueprintsLoading;

  // Load data on mount if stores are empty
  ...
  }, [setNodes, setNodesLoading, addToast]);
  ...
  }, [setBlueprints, setBlueprintsLoading, addToast]);

  useEffect(() => {
    loadNodes();
    loadBlueprints();
  }, [loadNodes, loadBlueprints]);
