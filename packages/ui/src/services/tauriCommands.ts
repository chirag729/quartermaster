...
import type { TaskInfo, TaskStateInfo, InstalledTaskState } from "@quartermaster/core";
import type { DenialEvent, PermissionSuggestion, ProfileInfo, ProfileDetail, ApplyPermissionsRequest, ConsolidationResult, ProfileTemplateInfo, ProfileTemplateConfig, SyncResultInfo } from "@quartermaster/core";
import type { Node, NodeKind, NodeStatus, SshConfig, SshHostEntry, YubiKeyInfo, Fido2Credential } from "@quartermaster/core";
...
export async function deployKeyToNode(nodeId: string, publicKeyPath: string): Promise<void> {
  return invoke("deploy_key_to_node", { nodeId, publicKeyPath });
}
...
export interface Fido2DependencyStatus {
  libfido2_installed: boolean;
  ykman_installed: boolean;
  all_satisfied: boolean;
  install_attempted: boolean;
  install_output: string | null;
}

export async function checkFido2Dependencies(): Promise<Fido2DependencyStatus> {
  return invoke("check_fido2_dependencies");
}

export async function installFido2Dependencies(): Promise<Fido2DependencyStatus> {
  return invoke("install_fido2_dependencies");
}

// YubiKey setup / conversion commands
export interface RemoteSshVersionInfo {
  version_string: string;
  major: number;
  minor: number;
  supports_sk_keys: boolean;
}

export async function checkRemoteSshVersion(nodeId: string): Promise<RemoteSshVersionInfo> {
  return invoke("check_remote_ssh_version", { nodeId });
}

export interface SshdHardenConfig {
  disable_password_auth: boolean;
  disable_challenge_response: boolean;
  disable_pam: boolean;
}

export interface SshdHardenResult {
  backup_path: string;
  changes_made: string[];
  sshd_restarted: boolean;
  warnings: string[];
}

export async function hardenRemoteSshd(nodeId: string, config: SshdHardenConfig): Promise<SshdHardenResult> {
  return invoke("harden_remote_sshd", { nodeId, config });
}
