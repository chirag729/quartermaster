...
import { useBlueprintStore, useFleetStore } from "@/hooks/useStore";
...
export function BlueprintsPage() {
  const { blueprints, loading } = useBlueprintStore();
  ...
  const handleCreateBlueprint = async (name: string, description: string, icon?: string) => {
    setCreating(true);
    try {
      const bp = await blueprintService.createBlankBlueprint(name, description, icon);
      setShowCreate(false);
  ...
  const handleImportBlueprint = async (path: string) => {
    setImporting(true);
    try {
      const bp = await blueprintService.importBlueprintPackage(path);
      setShowImport(false);
