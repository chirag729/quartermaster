...
import { formatError, taskService } from "@quartermaster/core";
...
export function TaskDetailPage() {
  ...
  // Derive task from the global task store
  ...
  // Track uninstall progress from task store
  useEffect(() => {
    if (!taskId || uninstalling !== taskId) return;
    const match = tasks.find((t) => t.id === taskId);
    if (match?._progressMessage) {
      setUninstallProgress(match._progressMessage);
    }
  }, [taskId, tasks, uninstalling]);
  ...
  const handleUninstall = async () => {
    ...
    try {
      await taskService.uninstallTask(taskId);
