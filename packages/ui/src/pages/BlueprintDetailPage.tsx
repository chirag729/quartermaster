...
import { getInstallChain, getUninstallChain } from "@quartermaster/core";
...
export function BlueprintDetailPage() {
  ...
  const handleClone = async () => {
    ...
    if (!newName) return;
    try {
      const cloned = await blueprintService.cloneBlueprint(blueprint.id, newName);
  ...
  const handleExport = async () => {
    ...
    try {
      const homeDir = "~";
      const outputPath = await blueprintService.exportBlueprintPackage(blueprint.id, homeDir);
